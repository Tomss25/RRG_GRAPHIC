...
def _read_csv_autodetect(raw: bytes) -> "pd.DataFrame":
    """Auto-detect CSV separator and decimal (handles European ; + comma-decimal format)."""
...
def parse_file(uploaded) -> pd.DataFrame:
    ...
    # Fallback: handle residual comma-as-decimal in object columns
    for col in df.columns:
...
def build_rrg_figure(
    ...
    # Ensure axis ranges are clean Python floats (no NaN/inf)
    def safe_range(lo, hi):
        import math
        if any(v is None or (isinstance(v, float) and not math.isfinite(v)) for v in [lo, hi]):
            return None
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            title=dict(
                text="RS-Ratio  →  (Forza Relativa)",
                font=dict(size=11, color="#4A5A7A"),
            ),
            gridcolor="rgba(10,22,40,0.06)",
            tickfont=dict(family="DM Mono", size=10, color="#8A9BBE"),
            zeroline=False,
            range=safe_range(xmin, xmax),
        ),
        yaxis=dict(
            title=dict(
                text="RS-Momentum  ↑  (Velocità)",
                font=dict(size=11, color="#4A5A7A"),
            ),
            gridcolor="rgba(10,22,40,0.06)",
            tickfont=dict(family="DM Mono", size=10, color="#8A9BBE"),
            zeroline=False,
            range=safe_range(ymin, ymax),
        ),
        legend=dict(
            orientation="h",
        ...
        hoverlabel=dict(
            bgcolor="#0A1628",
            font=dict(color="white", family="DM Mono", size=12),
            bordercolor="#0A1628",
        ),
        height=620,
    )
