...
def parse_file(uploaded) -> pd.DataFrame:
    ...
    for col in df.columns:
        if "date" in str(col).lower() or "data" in str(col).lower():
            date_col = col
            break
    if date_col is None:
        try:
            ...
            raise ValueError("Nessuna colonna data trovata. Assicurati che la prima colonna contenga date.")
...
with st.sidebar:
    ...
    if df_raw is not None:
        ...
        with col1:
            range_from = st.date_input("Da", value=date_min, min_value=date_min, max_value=date_max)
        with col2:
            range_to = st.date_input("A",  value=date_max, min_value=date_min, max_value=date_max)
